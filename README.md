...
Finally, the `LSQ` is unique in the sense that it can only execute in **sequential order**, that is, the `LSQ` does **not** execute instructions dispatched to it out-of-order. This is because Memory addresses are not aliased like registers, and therefore cannot be handled out-of-order like registers.v 
